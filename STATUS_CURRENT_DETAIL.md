...
1.  **`BatchReader` Parity**:
    ...
    *   [x] **Repetition Levels**: Added skipping logic for nested types.
    *   [x] **Full Type Coverage**: Added `INT96` (timestamps) and `FIXED_LEN_BYTE_ARRAY` (Decimals/UUIDs) support. Verified on official Parquet test files (`int96_from_spark.parquet`, `fixed_length_decimal.parquet`).
2.  **CLI Refactor**:
