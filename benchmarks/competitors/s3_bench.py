...
endpoint_override = os.environ.get("S3_ENDPOINT_URL") or os.environ.get("AWS_ENDPOINT_URL")

print(f"Benchmarking S3 Read: {PATH} (Mode: {MODE})")
if endpoint_override:
    print(f"Endpoint: {endpoint_override}")

# Shared S3 Filesystem for PyArrow (C++ Native)
if endpoint_override:
    # Strip https:// if present for PyArrow
    endpoint = endpoint_override.replace("https://", "").replace("http://", "")
    fs = pyarrow.fs.S3FileSystem(region=region, endpoint_override=endpoint)
else:
...
if HAS_POLARS:
    ...
    if endpoint_override:
        storage_options["endpoint_url"] = endpoint_override
