...
const KEY_SMALL_DEFAULT = "test_data/valid/sizes/small/10k_rows.parquet";
// Large file that hangs
const KEY_LARGE_DEFAULT = "raw/zpq/test-data/zpq-large.snappy.parquet";
...
pub fn main() !void {
    ...
    const bucket = std.posix.getenv("S3_BUCKET") orelse "zpq-staging-data";
    const key_small = std.posix.getenv("S3_KEY_SMALL") orelse KEY_SMALL_DEFAULT;
    const key_large = std.posix.getenv("S3_KEY_LARGE") orelse KEY_LARGE_DEFAULT;
    ...
    const result1 = testHeadRequest(allocator, bucket, key_small, config);
    if (result1) |size| {
    ...
    std.debug.print("Key: {s}\n", .{key_large});
    const result2 = testHeadRequest(allocator, bucket, key_large, config);
    if (result2) |size| {
        std.debug.print("SUCCESS: Content-Length = {d} bytes\n\n", .{size});
    } else |err| {
        std.debug.print("FAILED: {}\n\n", .{err});
...
fn testHeadRequest(allocator: std.mem.Allocator, bucket: []const u8, key: []const u8, config: types.S3Config) !u64 {
