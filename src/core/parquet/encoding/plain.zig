...
pub fn Decoder(comptime T: type) type {
    ...
    return struct {
        ...
        pub fn decode(self: *Self, dest: []T) Error!usize {
            ...
            // PLAIN is just little-endian native bytes; on a little-
            // endian host the decode is a `memcpy(n*elem_size bytes)`.
            // The pre-existing per-value loop didn't vectorize cleanly
            // under LLVM (a u32 readInt + bitcast confused the auto-
            // vectorizer enough to fall back to scalar). Microbench
            // (2026-05-07) showed numeric PLAIN was 8 ns/value for
            // f64/i64 — well below the ~30 GB/s the host's memcpy
            // routine can hit. Direct memcpy closes the gap.
            const native_endian = comptime @import("builtin").cpu.arch.endian();
            if (native_endian == .little) {
                const byte_count = n * elem_size;
                const dst_ptr: [*]u8 = @ptrCast(dest.ptr);
                @memcpy(dst_ptr[0..byte_count], self.bytes[self.pos..][0..byte_count]);
                self.pos += byte_count;
                return n;
            }
