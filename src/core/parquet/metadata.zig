...
pub fn findColumnIndex(
    ...
    // Schema is in DFS order. We want the LEAF index (i.e. the index
    // into row_group.columns[]), so we count only primitive leaves and
    // skip group nodes. A leaf has a non-null `type`; group nodes have
    // `type == null` and `num_children > 0`.
    //
    // For a flat schema this matches "first non-root index" because
    // there are no groups. For nested schemas (struct / list / map
    // produces hidden GroupNode wrappers in the Parquet thrift schema)
    // counting all non-root elements would shift leaf indices off,
    // which mis-aligns lookups against row_group.columns[]. Discovered
    // 2026-05-06 against DuckDB-written parquet where Map<string, X>
    // columns introduce GroupNodes — a target DOUBLE column was the
    // 31st leaf but the 35th schema element. Reading rg.columns[35]
    // returned a different DOUBLE column without dict_page_offset →
    // dictionary decode silently produced garbage values.
    var leaf_idx: usize = 0;
    for (file.schema.items[1..]) |elem| {
        if (elem.type == null) continue; // group node — has children, not a column-chunk
