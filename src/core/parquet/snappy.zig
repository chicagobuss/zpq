...
///
/// Delegates to vendor/snappy (google/snappy) via the FFI module —
/// the hand-rolled Zig path below is kept as a fallback (and as
/// behavioral documentation), but google's reference implementation
/// is materially faster on the byte-bound decode hot path. Microbench
/// (2026-05-07) showed numeric PLAIN/RLE_DICTIONARY columns sitting
/// at ~1.2 GB/s decoded throughput against benchmark_100mb.parquet
/// — most of that floor was here.
pub fn uncompress(src: []const u8, dest: []u8) !usize {
    return c_snappy.uncompress(src, dest) catch |err| switch (err) {
        error.OutputTooSmall => error.OutputTooSmall,
        error.InvalidInput => error.CorruptInput,
    };
}

/// The hand-rolled Zig snappy decoder. Kept around so the test suite
/// continues to exercise the algorithm directly (cross-checks against
/// the C version) and as a no-FFI fallback if we ever want one. NOT
/// the production path; that's `uncompress` above.
pub fn uncompressZig(src: []const u8, dest: []u8) !usize {
...
fn emitCopy(dest: []u8, offset: usize, length: usize) CompressError!usize {
    ...
    while (len > 0) {
        if (offset < 2048 and len >= 4 and len <= 11) {
            ...
            // (2) gives a byte in [2..254]. Keep the shift operands wide:
            // if result-type inference narrows the shift to u8, copy_len
            // > 32 loses the 0x80 bit and produces a wrong tag byte
            // (e.g. 0x12 instead of 0x92 for copy_len=37).
...
test "snappy compress regression: size=294 k%256 pattern" {
    ...
    const dec_len = try uncompress(compressed, out);
    if (dec_len != size) {
        std.debug.print("\nsnappy roundtrip: size={d} dec_len={d} compressed_len={d}\n", .{ size, dec_len, compressed.len });
        std.debug.print("last 30 compressed bytes (offsets {d}..{d}): ", .{ compressed.len - 30, compressed.len });
...
test "snappy compress: copy-byte tag-narrowing regression sweep" {
    // Sweep many sizes that trigger long backward-references. Each
    // compressed output must decompress to the original bytes.
    const allocator = std.testing.allocator;
    var size: usize = 200;
