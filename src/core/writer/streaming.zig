    ...
    for (files) |f| {
        ...
        for (f.survivors, 0..) |keep, i| {
            ...
            if (kept_columns) |kc| {
                ...
                const cloned = try cloneRowGroupShifted(arena, sink, &offset, f.bytes, rg, delta);
...
fn writeProjectedRowGroup(
    ...
    for (kept) |col_idx| {
        ...
        // Carry the page index forward — written to the sink right after
        // this chunk's data, with offsets rebased by the chunk's delta.
        const idx = try carryPageIndexStreaming(arena, sink, offset, input, &src_chunk, delta);
...
fn cloneRowGroupShifted(
    ...
    for (src.columns.items) |chunk| {
        ...
        // Whole-RG copy: every chunk in the group moved by the same
        // delta, so the rebased index is written here, after the RG data.
        const idx = try carryPageIndexStreaming(arena, sink, offset, input, &chunk, delta);
        new_chunk.offset_index_offset = idx.offset_index_offset;
        new_chunk.offset_index_length = idx.offset_index_length;
        new_chunk.column_index_offset = idx.column_index_offset;
        new_chunk.column_index_length = idx.column_index_length;
...
/// Streaming counterpart of `fastpath.carryPageIndex`: re-serialize the
/// source chunk's page index (ColumnIndex verbatim, OffsetIndex offsets
/// rebased by `delta`) straight to the sink, advancing `offset`. Returns
/// the new pointers; absent/unfetched/unparseable indexes yield nulls.
fn carryPageIndexStreaming(
    arena: std.mem.Allocator,
    sink: Sink,
    offset: *usize,
    input: []const u8,
    ...
    if (src.column_index_offset) |co| if (src.column_index_length) |cl| {
        if (fastpath.sliceInBounds(input, co, cl)) |bytes| {
            if (fastpath.reserializeColumnIndex(arena, bytes)) |ser| {
                const off = offset.*;
                try sink.write(ser);
                offset.* += ser.len;
    ...
    if (src.offset_index_offset) |oo| if (src.offset_index_length) |ol| {
        if (fastpath.sliceInBounds(input, oo, ol)) |bytes| {
            if (fastpath.reserializeOffsetIndexShifted(arena, bytes, delta)) |ser| {
                const off = offset.*;
                try sink.write(ser);
                offset.* += ser.len;
