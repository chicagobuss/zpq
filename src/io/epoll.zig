//! epoll-based event loop. Phase A.
//!
//! Contract: docs/event_loop_design.md.
//!
//! Key design choices (read the doc for the why):
...
pub const Loop = struct {
    ...
    fn runInner(self: *Loop, timeout_ms: i32) RunError!void {
        // Phase 1: dispatch any callbacks that are already queued from the
        ...
        // Phase 2: flush submissions.
        self.flushSubmissions() catch return error.Unexpected;

        // Phase 3: wait for events. If there's nothing in flight, skip
        ...
        // Phase 4: dispatch newly-completed callbacks.
        self.dispatchCompleted();
    }
        ...
        while (self.submissions.pop()) |c| {
            switch (c.op) {
                ...
                .connect => |conn| {
                    ...
                    // immediately we still go through epoll for uniformity.
