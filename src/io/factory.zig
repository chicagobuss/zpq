...
pub fn openSink(allocator: std.mem.Allocator, path: []const u8, options: FactoryOptions) !sink_mod.Sink {
    if (std.mem.startsWith(u8, path, "s3://")) {
        const bucket_end = std.mem.indexOfScalarPos(u8, path, 5, '/') orelse return error.InvalidS3Path;
        const bucket = path[5..bucket_end];
        const key = path[bucket_end + 1 ..];
        ...
        const sink_ptr = try S3Sink.init(allocator, @ptrCast(options.loop.?), options.thread_pool.?, s3_config, bucket, key);
