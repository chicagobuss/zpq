...
pub const SigV4 = struct {
    ...
    pub fn sign(
        ...
        // Buffer sized for "AWS4" + secret key. AWS keys are 40 chars
        // (= 44), R2 keys are 64-char hex (= 68). 256 leaves headroom
        // for any S3-compatible provider with a longer secret.
...
test "sign accepts a 64-char secret (R2 key length)" {
    // R2 secret access keys are 64-char hex strings, longer than AWS's
    // 40-char keys. The "AWS4" + secret buffer must be sized from the
    // input secret, not an AWS-specific constant.
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();

    const long_secret = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
    try testing.expectEqual(@as(usize, 64), long_secret.len);
