...
//! invocation), so non-blocking buys us nothing and adds bug surface.
...
//!     Posts an init failure. Used if we can't even start the loop.
//!     We don't post these today — fatal init errors just exit.
