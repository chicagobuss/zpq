//! Per-file streaming Parquet scan iterator.
//!
...
//! is the input-side counterpart of the streaming output sink.
//!
//! Scope: lambda-specific because Lambda owns the S3 fan-out and warm
//! connection pool. Keep it here until another caller needs the same
//! state machine.
//!
...
//! same primitive used elsewhere for range fetches). Fetching one row
//! group at a time preserves the memory ceiling while still letting the
//! pool fan out within that row group's column ranges.
...
pub const PerFileScan = struct {
    ...
    pub fn next(self: *PerFileScan, io: Io, arena: std.mem.Allocator) !?RowGroupResult {
        ...
        // uses for equivalent range windows, keeping scan accounting
