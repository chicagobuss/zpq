...
pub const Pipeline = struct {
    ...
    pub fn execute(self: *Self, mode: ExecutionMode) !ExecutionResult {
        ...
        const effective_mode = if (mode == .slot_parallel and self.output_path != null and
            std.mem.startsWith(u8, self.output_path.?, "s3://")) .morsel_parallel else mode;
        ...
        return switch (effective_mode) {
            ...
            .morsel_parallel => self.executeMorselParallel(),
        };
    }
    ...
    pub fn executeWithLoop(self: *Self, comptime XevApi: type, loop: *XevApi.Loop, pool: *xev.ThreadPool, mode: ExecutionMode) !ExecutionResult {
        ...
        return switch (effective_mode) {
            ...
            .morsel_parallel => self.executeMorselParallelWithLoop(XevApi, loop, pool),
        };
    }
    ...
    /// Morsel-parallel execution: direct S3 multipart upload without temp file staging.
    /// Each row group is uploaded as a separate part in parallel.
    fn executeMorselParallel(self: *Self) !ExecutionResult {
        const loop = self.loop orelse return error.NoRuntime;
        const pool = self.thread_pool orelse return error.NoRuntime;
        return self.executeMorselParallelWithLoop(xev.Dynamic, loop, pool);
    }

    /// Generic morsel-parallel execution with any xev backend.
    fn executeMorselParallelWithLoop(self: *Self, comptime XevApi: type, loop: *XevApi.Loop, thread_pool: *xev.ThreadPool) !ExecutionResult {
        ...
        // Pre-fetch all row group data (same as slot_parallel)
        var all_rg_data = try self.allocator.alloc(RowGroupData, rg_count);
        defer self.allocator.free(all_rg_data);
        ...
        for (all_rg_data, 0..) |*rg_data, rg_idx| {
            ...
            // Create worker and execute (filter + decode)
            ...
            // Skip if no rows matched filter
            if (worker.row_count == 0) continue;

            total_output_rows += worker.row_count;

            // Encode the filtered data to a buffer
            ...
            // Build morsel metadata from slot_writer column metas
            ...
            // Submit to coordinator (handles backpressure)
        ...
        if (!success) {
            return error.MorselUploadFailed;
        }
