...
pub const XevConnectionPool = struct {
    allocator: std.mem.Allocator,
    idle_connections: std.ArrayListUnmanaged(Entry) = .{},
    max_idle: usize = 64, // Match max_concurrency in xev_source.zig
    ...
    pub fn init(allocator: std.mem.Allocator) XevConnectionPool {
        return .{
            .allocator = allocator,
        };
    }
    ...
    pub fn acquire(self: *XevConnectionPool, key: ConnectionKey) ?*Connection {
        log.debug("[POOL] acquire: pool has {d} idle connections", .{self.idle_connections.items.len});
    ...
    pub fn release(self: *XevConnectionPool, key: ConnectionKey, conn: *Connection) !void {
        log.debug("[POOL] release: adding connection to pool (now {d})", .{self.idle_connections.items.len + 1});
