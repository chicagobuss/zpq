        ...
        while (i < merged_list.items.len) {
            ...
            for (batch_merged, 0..) |merged, j| {
                ...
                const conn = if (self.pool.acquire(key)) |c| blk: {
                    ...
                    c.* = try tls.Connection.initWithOptions(self.loop, self.allocator, self.host, .{ .verify_certificate = self.verify_certificate });
                    c.tcp_read_buf_size = self.tcp_read_buf_size;
                    c.use_direct = self.use_direct;
                    break :blk c;
                };
...
fn onConnect(ctx_void: ?*anyopaque) void {
    ...
    if (ctx.source.access_key) |ak| {
        if (ctx.source.secret_key) |sk| {
            ...
            const url = std.fmt.allocPrint(aa, "{s}://{s}{s}", .{ scheme, ctx.source.host, path }) catch |err| {
                ctx.err = err;
                ctx.conn.close();
                return;
            };
...
fn onData(ctx_void: ?*anyopaque, data: []const u8) void {
    ...
    if (ctx.is_head and ctx.parser.headersComplete()) {
        ...
        ctx.source.pool.release(.{ .host = ctx.source.host, .port = ctx.source.port, .use_tls = ctx.source.use_tls }, ctx.conn);
        if (ctx.batch_ctx) |bc| {
