...
pub fn eq_i32(allocator: std.mem.Allocator, left: Vector, right: Vector) !Vector {
    ...
    while (i + 8 <= len) : (i += 8) {
        ...
        // Convert bool vector to u8 vector (0 or 1)
        // Zig @select or internal casting?
        // @intFromBool is scalar.
        // We can do manual loop or @select.
        // For auto-vectorization, a loop is often fine.
        // But let's try to be explicit if possible.
        // Actually, for @Vector(N, bool), just iterating is probably fine for this prototype.
