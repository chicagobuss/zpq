...
pub const VectorRowGroupWorker = struct {
    ...
    pub fn init(
        ...
        var prefetched_buffers: std.ArrayList([]u8) = .empty;
        errdefer {
            for (prefetched_buffers.items) |b| allocator.free(b);
            prefetched_buffers.deinit(allocator);
        }

        var prefetched_sources: std.ArrayList(*interface.local.MemorySource) = .empty;
        errdefer {
            for (prefetched_sources.items) |s| allocator.destroy(s);
            prefetched_sources.deinit(allocator);
        }
        ...
        var ranges: std.ArrayList(interface.Range) = .empty;
        ...
        for (row_group.columns.items, 0..) |chunk, i| {
            ...
            if (total_size > 0 and total_size < PREFETCH_THRESHOLD) {
                ...
                try prefetched_buffers.append(allocator, buf);

                try ranges.append(allocator, .{ .start = start_offset, .end = start_offset + total_size });
        ...
        for (row_group.columns.items, 0..) |chunk, i| {
            ...
            if (col_prefetch_map[i]) |buf_idx| {
                ...
                try prefetched_sources.append(allocator, mem_src);
                source_to_use = mem_src.source();
            }

            const col_reader = try column.ColumnReader.init(source_to_use, chunk);
            readers[i] = VectorColumnReader.init(allocator, col_reader, col_meta.type, @intCast(levels.max_def), @intCast(levels.max_rep));
        }
    ...
    pub fn deinit(self: *VectorRowGroupWorker) void {
        ...
        self.prefetched_sources.deinit(self.allocator);

        for (self.prefetched_buffers.items) |b| self.allocator.free(b);
        self.prefetched_buffers.deinit(self.allocator);
    }
