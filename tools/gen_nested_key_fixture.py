...
The relevant nested-schema shape is cheap to reproduce: a MAP column appearing BEFORE ordinary scalar columns.

`col_idx` counts primitive leaves, but `meta.schema` is a DFS that also holds the group nodes a MAP introduces (the map
node itself and its `key_value` node). So from the first MAP onward, leaf numbering and DFS numbering drift, and any
code indexing the schema by leaf index reads the wrong element. A TIMESTAMP column resolving to a STRING element made
key deserialization read the timestamp's own bytes as a length.
    ...
    table = pa.table(
        {
            ...
            # Independent of `flag`, so a composite (flag, name) key really has 2 x 4 groups; keying both off i % 2
            # would collapse it to 4 and a truncated string half would still look plausible.
