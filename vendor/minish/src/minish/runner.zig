...
pub fn check(
    ...
    // We just need *some* chaos for the seed, using internal representation is fine
    // Since Instant.timestamp is opaque/platform-dependent, we hash it
    var hasher = std.hash.Wyhash.init(0);
    std.hash.autoHash(&hasher, timestamp);
    const seed = options.seed orelse hasher.final();
    ...
    while (i < options.num_runs) : (i += 1) {
        ...
        const T = @TypeOf(test_fn);
        const result = if (@typeInfo(T) == .@"struct" and @hasDecl(T, "run"))
            test_fn.run(value)
        else if (@typeInfo(T) == .@"fn") test_fn(value) else unreachable; // Force branch pruning
        ...
        result catch |err| {
            ...
            if (generator.shrinkFn) |shrinker| {
                ...
                while (it.next()) |next_val| {
                    ...
                    const T_shrink = @TypeOf(test_fn);
                    const result_shrink = if (@typeInfo(T_shrink) == .@"struct" and @hasDecl(T_shrink, "run"))
                        test_fn.run(next_val)
                    else
                        test_fn(next_val);
